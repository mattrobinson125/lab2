...
Creating a count for fatal shootings per day (not needed)
```{r}
...
Here is another twist on the fatal police shootings data. 

Selecting the data that only occurred in 2015
```{r}
year2015 = fatalshootings %>% slice(1:990)
```
...
ageinfo <- fatalshootings %>% group_by(age) %>% summarise(n = n()) %>% 
        arrange(desc(n)) %>% top_n(10) %>%   
        mutate(age = factor(age, levels = rev(unique(age))))

armedage = bind_cols(armedinfo, ageinfo)
```
