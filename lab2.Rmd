...
In 2016, The Washington post went to gather additional information about each fatal shooting that occurred during this year. There were reports, websites, and social media that have been constantly monitoring independent databases about fatal police shootings. More than a dozen additional details were collected about the officers and victims in each individual shooting. As of right now, there is continuous live web tracking to recording the number of Fatal Police Shootings in 2016.
...
```
Clearly, male victims fall into the greater number of fatal police shootings. Further investigation is needed since the male ratio deaths are overwhelmingly higher than the females.
```
...
```
The histogram forms a skewed-right distribution. The average number of deaths seem to be the greatest around the age of 30. We are also glad that children are not usually involved wth fatal shootings. Overall, the creator applies good data visualization by using consistent themes and having a clear number of bins for her ggplots.
```
...
```
This graph is a little bit more complicated than the rest. Julie focuses on seperating the top 15 states when finding the unique number of variables. She creates a state column and then maps it to the barplot. She creates the size and identifies the top 15 Deaths by State. Originally there were state icons, but we couldn't find the 'geom_stateface' asset. 
```
...
```
There were a great number of unusually recored weapons. It makes sense that the data describes most fatal shootings would occur when victim holding out a gun. However, it does not make sense when they are completely unarmed.
```
...
```
There should be more officers wearing a body camera. Somehow I believe there were faulty reports and censored data collection when asked if there were a body camera on the approaching officer after shooting a victim to death.
```
...
Another twist on the fatal police shootings data.
...
Simple analysis without making graphical models 
```{r}
...
Creating a count for fatal shootings per day
...
#### Random Linear Model of Fatal Police Shootings Per Day 
```{r}
...
- There was the opportunity to reorder Julie's graph in order to interpret some other underlying  information
```
